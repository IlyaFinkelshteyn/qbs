...
UnixBase {
    ...
    Rule {
        ...
        prepare: {
            ...
            var commands = [];
            var i;
            var args = Compiler.configFlags(product);
            args.push('-shared');
            if (product.moduleProperty("qbs", "targetOS").contains('linux')) {
                args = args.concat([
                    '-L--hash-style=gnu',
                    '-L--as-needed',
                    '-L-soname=' + Unix.soname(product, libFilePath)
                ]);
            ...
            args = args.concat(Compiler.linkerFlags(product, inputs));
            args = args.concat(Compiler.additionalCompilerAndLinkerFlags(product));
