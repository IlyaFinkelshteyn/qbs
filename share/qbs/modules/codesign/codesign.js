...
function findBestProvisioningProfile(product, files) {
    ...
    var profilePlatform = product.moduleProperty("codesign", "_provisioningProfilePlatform");
...
function outputArtifacts(product, input, fileTag) {
    var artifacts = [{
        filePath: input.moduleProperty("qbs", "_moduleData_filePath"),
        fileTags: [fileTag, "codesign.signed_artifact"]
    }];
    if (product.moduleProperty("qbs", "targetOS").contains("darwin")
            && product.moduleProperty("codesign", "enableCodeSigning")
            && product.moduleProperty("bundle", "isBundle")
            && fileTag === "bundle") {
        artifacts.push({
            filePath: FileInfo.joinPaths(product.moduleProperty("bundle",
                                                                "contentsFolderPath"),
                                         "_CodeSignature", "CodeResources"),
            fileTags: ["bundle.codesignature"]
        });
    }
    return artifacts;
}
