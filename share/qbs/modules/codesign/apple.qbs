...
CodeSignModule {
    Depends { name: "bundle" } // for bundle.identifier in automatic provisioning
    Depends { name: "xcode"; condition: qbs.hostOS.contains("osx") }
    ...
    codesignPath: FileInfo.joinPaths("/usr/bin", codesignName)

    property string securityName: "security"
    property string securityPath: FileInfo.joinPaths("/usr/bin", securityName)
    ...
    property string signingIdentity: {
        if (signingType === "ad-hoc") // only useful on OS X
            return "-";

        var d = qbs.buildVariant === "debug";

        if (qbs.targetOS.contains("ios") || qbs.targetOS.contains("tvos")
                || qbs.targetOS.contains("watchos")) {
            switch (signingType) {
            case "app-store":
                return d ? "iPhone Developer" : "iPhone Distribution";
            }
        }
        ...
        if (qbs.targetOS.contains("osx")) {
            switch (signingType) {
            case "app-store":
                return d ? "Mac Developer" : "3rd Party Mac Developer Application";
            case "apple-id":
    ...
    readonly property bool _codeSigningRequired:
        XcodeUtils.boolFromSdkOrPlatform("CODE_SIGNING_REQUIRED",
    ...
    readonly property bool _provisioningProfileAllowed:
        bundle.isBundle && product.type.contains("application") && !xcode.isSimulatorPlatform
    ...
    readonly property bool _provisioningProfileRequired:
        _provisioningProfileAllowed && !qbs.targetOS.contains("osx")
    ...
    Rule {
        multiplex: true
        condition: product.moduleProperty("codesign", "enableCodeSigning") &&
                   product.moduleProperty("codesign", "_provisioningProfileAllowed")
        inputs: ["qbs", "codesign.provisioningprofile"]

        outputFileTags: ["codesign.embedded_provisioningprofile"]
        outputArtifacts: {
            var artifacts = [];
            var provisioningProfiles = (inputs["codesign.provisioningprofile"] || []).map(function (a) { return a.filePath; });
            var bestProfile = CodeSign.findBestProvisioningProfile(product, provisioningProfiles);
            var uuid = ModUtils.moduleProperty(product, "provisioningProfile");
            if (bestProfile) {
                artifacts.push({
                    filePath: FileInfo.joinPaths(product.destinationDirectory,
                                                 ModUtils.moduleProperty(product,
                                                                         "_embeddedProfileName")),
                    fileTags: ["codesign.embedded_provisioningprofile"],
                    qbs: {
                        _moduleData_provisioningProfileFilePath: bestProfile.filePath,
                        _moduleData_provisioningProfileData: JSON.stringify(bestProfile.data),
                    }
                });
            } else if (uuid) {
                throw "Your build settings specify a provisioning profile with the UUID “" + uuid + "”, however, no such provisioning profile was found.";
            } else if (ModUtils.moduleProperty(product, "_provisioningProfileRequired")) {
                var hasProfiles = !!((inputs["codesign.provisioningprofile"] || []).length);
                var teamIdentifier = ModUtils.moduleProperty(product, "teamIdentifier");
                var codeSignIdentity = ModUtils.moduleProperty(product, "signingIdentity");
                if (hasProfiles) {
                    if (codeSignIdentity)
                        console.warn("No provisioning profiles matching the bundle identifier “" + product.moduleProperty("bundle", "identifier") + "” were found.");
                    else
                        console.warn("No provisioning profiles matching an applicable signing identity were found.");
                } else {
                    if (codeSignIdentity) {
                        if (teamIdentifier)
                            console.warn("No provisioning profiles with a valid signing identity (i.e. certificate and private key pair) matching the team ID “" + teamIdentifier + "” were found.")
                        else
                            console.warn("No provisioning profiles with a valid signing identity (i.e. certificate and private key pair) were found.");
                     } else {
        ...
        prepare: {
            ...
            var data = JSON.parse(output.moduleProperty("qbs", "_moduleData_provisioningProfileData"));
            cmd.source = output.moduleProperty("qbs", "_moduleData_provisioningProfileFilePath");
    ...
    Rule {
        multiplex: true
        condition: product.moduleProperty("codesign", "enableCodeSigning")
        inputs: ["qbs", "codesign.entitlements", "codesign.embedded_provisioningprofile"]
        ...
        prepare: {
            ...
            cmd.signingEntitlements = (inputs["codesign.entitlements"] || []).map(function (a) { return a.filePath; });
            cmd.provisioningProfiles = (inputs["codesign.embedded_provisioningprofile"] || []).map(function (a) { return a.filePath; });
            cmd.platformPath = product.moduleProperty("xcode", "platformPath");
            cmd.sdkPath = product.moduleProperty("xcode", "sdkPath");
            cmd.sourceCode = function() {
                var i;
                var provData = {};
                ...
                for (i in provisionProfiles) {
                    ...
                    try {
                        plist.readFromData(qbs.internal.smimeMessageContent(
