...
{
public:
    QString buildToolPath = QStringLiteral("/bin/bash");
    QString buildArgumentsString = QStringLiteral("$(ACTION)");
    QString buildWorkingDirectory = QStringLiteral("$BUILT_PRODUCTS_DIR");
    bool passBuildSettingsInEnvironment = true;
};
...
{
    return QStringLiteral("PBXLegacyTarget");
}

PBXObjectMap PBXLegacyTarget::toMap() const
{
    PBXObjectMap self = PBXTarget::toMap();
    self.insert(QStringLiteral("buildToolPath"), d->buildToolPath);
    self.insert(QStringLiteral("buildArgumentsString"), d->buildArgumentsString);
    self.insert(QStringLiteral("buildWorkingDirectory"), d->buildWorkingDirectory);
    self.insert(QStringLiteral("passBuildSettingsInEnvironment"), d->passBuildSettingsInEnvironment ? 1 : 0);
    return self;
}
